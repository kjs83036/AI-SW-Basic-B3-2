# Mini Git 구축 관련 컴퓨터 사이언스(CS) 이론 상세 보고서 (버전 5)

본 보고서는 `cs_theory_report_v4.md` 문서를 기반으로 작성된 **버전 5 최종 확장판**입니다. 컴퓨터 사이언스(CS) 배경지식이 없는 고등학생도 각 이론의 원리와 역할을 쉽게 파악할 수 있도록 친절한 비유가 풍부하게 포함되어 있으며, 이번 버전에서는 `minigit_구축.md` 문서의 주요 요구사항 및 명령어별로 적용되는 CS 이론 매핑 정보를 추가하였습니다.
...
## 1. Mini Git 요구사항(구축 가이드) 대비 CS 이론 매핑

`minigit_구축.md` 문서에 기재된 주요 요구사항 및 명령어별로 적용된 컴퓨터 사이언스(CS) 이론의 매핑은 다음과 같습니다.

### 4. 기능 요구 사항
* **1. CLI 공통 규칙(문법 표준)**
    * 관련 적용이론: `[이론 8] REPL 파싱과 어휘 분석 (Lexical Analysis)` (shlex.split 등을 활용한 문법 파싱 및 따옴표 기호 처리)
* **2. 커밋 그래프(핵심 자료구조)**
    * 관련 적용이론: 
        * `[이론 1] DAG (Directed Acyclic Graph - 방향성 비순환 그래프)` (커밋 간의 역방향 연결 계보 구성)
        * `[이론 7] 해시 테이블 및 해싱 (Hash Table & Hashing)` (상수 시간 O(1) 조회를 위한 저장소 구성)
        * `[이론 14] 유닉스 시간(Unix Time)과 타임스탬프(Timestamp)` (커밋 메타데이터의 생성 일자 기록)
        * `[이론 15] 객체 불변성 (Object Immutability) 및 데이터 무결성 (Data Integrity)` (Commit 객체의 frozen=True 설정을 통한 속성 값 임의 변경 및 훼손 방지)
        * `[이론 18] 메모이제이션 및 캐싱 (Memoization & Caching)` (자식 캐시 맵 children_dict를 이용한 검색 고속화)
* **3. 역색인(Inverted Index)**
    * 관련 적용이론: `[이론 6] 역색인 (Inverted Index)` (메시지 키워드 및 작성자별 고속 조회를 위한 인덱스 사전 구축)
* **4. 정렬 알고리즘 직접 구현**
    * 관련 적용이론:
        * `[이론 5] 정렬 알고리즘 (Sorting Algorithms)` (평균/최악 시간복잡도 O(N log N)을 만족하는 Stable 병합 정렬)
        * `[이론 11] 퀵 정렬 (Quick Sort)` (피벗 기반 분할 정복 방식의 고속 정렬)
        * `[이론 12] 재귀(Recursion)와 콜 스택(Call Stack), 스택 오버플로우(Stack Overflow)` (정렬 알고리즘 분할 정복 시의 재귀 호출 구조)
* **5. 명령어 기능 요구**
    * **- `INIT <user_name>`**
        * 관련 적용이론: `[이론 13] Git 참조(References)와 HEAD 포인터` (초기 main 브랜치 생성 및 HEAD 포인터 지정)
    * **- `BRANCH <branch_name>`**
        * 관련 적용이론: `[이론 13] Git 참조(References)와 HEAD 포인터` (현재 HEAD 커밋을 가리키는 브랜치 포인터 꼬리표 추가)
    * **- `SWITCH <branch_name>`**
        * 관련 적용이론: `[이론 13] Git 참조(References)와 HEAD 포인터` (HEAD 포인터를 대상 브랜치 포인터로 이동)
    * **- `COMMIT <message>`**
        * 관련 적용이론: 
            * `[이론 13] Git 참조(References)와 HEAD 포인터` (HEAD 및 활성 브랜치 포인터 갱신)
            * `[이론 1] DAG (Directed Acyclic Graph)` (현재 HEAD를 부모 노드로 하는 신규 커밋 정점 생성)
            * `[이론 6] 역색인 (Inverted Index)` (커밋 생성 시 키워드/작성자 인덱스 사전 자동 갱신)
            * `[이론 7] 해시 테이블 및 해싱` (해시맵 commit_dict에 신규 커밋 등록)
            * `[이론 14] 유닉스 시간(Unix Time)과 타임스탬프(Timestamp)` (커밋의 timestamp 필드 기록)
    * **- `LOG`**
        * 관련 적용이론:
            * `[이론 2] 위상 정렬 (Topological Sort)` (자식 커밋보다 부모 커밋이 먼저 나열되도록 Kahn 알고리즘 적용)
            * `[이론 10] 자료구조 기초 (스택 Stack & 큐 Queue)` (Kahn 알고리즘 진입 차수 제어를 위한 대기 큐 활용)
    * **- `LOG --sort-by=date|author`**
        * 관련 적용이론:
            * `[이론 5] 정렬 알고리즘 (Sorting Algorithms)` (동일 기준에 대한 안정 정렬 보장 및 비교 정렬 한계 직면)
            * `[이론 11] 퀵 정렬 (Quick Sort)` or `[이론 5] 병합 정렬` (커스텀 구현된 정렬 알고리즘 활용)
            * `[이론 12] 재귀(Recursion)와 콜 스택(Call Stack)` (정렬 알고리즘의 분할 재귀 연산)
            * `[이론 14] 유닉스 시간(Unix Time)과 타임스탬프(Timestamp)` (날짜 비교를 위해 유닉스 초단위 정수값 대소 연산)
    * **- `PATH <commit1> <commit2>`**
        * 관련 적용이론:
            * `[이론 3] 최단 경로 알고리즘 (BFS)` (무방향 가중치 없는 최단 경로 탐색 및 사전순 정렬을 통한 Tie-breaker 해결)
            * `[이론 10] 자료구조 기초 (스택 Stack & 큐 Queue)` (BFS의 넓이 우선 탐색을 위해 큐 활용)
    * **- `ANCESTORS <commit_hash>`**
        * 관련 적용이론:
            * `[이론 4] 그래프 순회 및 조상 탐색 (DFS)` (특정 커밋에서 도달 가능한 모든 과거 이력 탐색)
            * `[이론 10] 자료구조 기초 (스택 Stack & 큐 Queue)` (LIFO 방식의 탐색 후보군 축적을 위해 스택 사용)
            * `[이론 12] 재귀(Recursion)와 콜 스택(Call Stack), 스택 오버플로우(Stack Overflow)` (수만 건의 이력 탐색 시 Call Stack 한계 극복을 위해 명시적 힙 메모리 스택 기반 반복적 DFS 구현)
    * **- `SEARCH <keyword>`**
        * 관련 적용이론: `[이론 6] 역색인 (Inverted Index)` (단어별 역색인 사전을 활용하여 순회 없는 O(1) 검색 수행)
    * **- `SEARCH --author=<name>`**
        * 관련 적용이론: `[이론 6] 역색인 (Inverted Index)` (작성자별 역색인 사전을 활용하여 순회 없는 O(1) 검색 수행)

### 공통 시스템 설계 및 품질 관리 요구사항
* **CLI 입력 제어 및 공통 데코레이터**
    * 관련 적용이론: `[이론 16] 소프트웨어 디자인 패턴 - 데코레이터 패턴 (Decorator Pattern)` (require_init 및 args_to_list를 활용한 문법/권한 전처리 위임)
* **콘솔 인터페이스(REPL)와 핵심 로직의 물리적 격리**
    * 관련 적용이론: `[이론 17] 프레젠테이션 레이어 분리 및 MVC (Presentation-Domain Separation)` (main.py의 GitShell UI와 mini_git.py의 Git 비즈니스 로직 분리 설계)
* **자식 노드 탐색 성능 최적화**
    * 관련 적용이론: `[이론 18] 메모이제이션 및 캐싱 (Memoization & Caching)` (children_dict를 활용하여 자식 노드 스캔 시의 O(V) 탐색을 O(1)로 고속화)
* **기능 명세서(SRS) 충족 테스트 및 회귀 방지**
    * 관련 적용이론: `[이론 19] 단위 테스트 (Unit Testing)와 자동 검증` (test_minigit.py의 TestMiniGitSRS를 통한 검증 자동화)

### 5. 보너스 과제 (선택)
* **1. Diff(간단 비교) 추가**
    * 관련 적용이론: `[이론 20] LCS (Longest Common Subsequence - 최장 공통 부분 수열)` (두 텍스트의 편집 거리 및 추가/삭제 줄 구분을 위한 전통적 동적 계획법 알고리즘)
* **2. Merge(브랜치 병합) 흉내내기**
    * 관련 적용이론:
        * `[이론 9] 3-Way Merge 및 공통 조상(Lowest Common Ancestor - LCA) 탐색` (두 분기점의 LCA를 구한 뒤 Base-Mine-Theirs 3방향 상태 대조 병합 진행)
        * `[이론 1] DAG (Directed Acyclic Graph)` (다중 부모를 갖는 병합 커밋(Merge Commit) 조립)
        * `[이론 3] 최단 경로 알고리즘 (BFS)` / `[이론 10] 자료구조 기초 (스택 Stack & 큐 Queue)` (LCA 역방향 탐색에 최단거리 BFS 및 큐 사용)
* **3. 정렬 알고리즘 성능 비교**
    * 관련 적용이론:
        * `[이론 5] 정렬 알고리즘 (Sorting Algorithms)` (각 정렬 알고리즘의 평균/최악 시간복잡도 비교)
        * `[이론 11] 퀵 정렬 (Quick Sort)` (퀵 정렬과 타 정렬 방식의 피벗 유무에 따른 성능 및 최악 시나리오 편차 분석)

---

## 2. Mini Git CS 이론 위치 매핑 트리 구조
...
│   ├── require_init() / args_to_list() ─────────── [이론 16] 데코레이터 패턴 (Decorator Pattern)
│   ├── GitShell 클래스 ───────────────────────────── [이론 17] 프레젠테이션 레이어 분리 (MVC)
...
│   ├── Git 클래스 ──────────────────────────────── [이론 17] 프레젠테이션 레이어 분리 (MVC)
...
│   ├── Commit 클래스 ───────────────────────────── [이론 1] DAG 노드 설계, [이론 14] 타임스탬프, [이론 15] 객체 불변성(frozen=True)
│   ├── Commit_graph 클래스 ─────────────────────── [이론 7] 해시 테이블(dict), [이론 18] 메모이제이션(자식 노드 캐싱)
...
│   ├── find_common_ancestor() ──────────────────── [이론 9] BFS 기반 LCA 탐색 & [이론 10] 큐
│   ├── find_shortest_path() ────────────────────── [이론 3] BFS 기반 최단 경로, [이론 18] 자식 캐시 활용
│   └── topological_sort() ──────────────────────── [이론 2] Kahn 알고리즘 위상 정렬 로직 & [이론 10] 큐 대기열 & [이론 18] 자식 캐시 활용
└── test_minigit.py (기능 요구 사항 단위 검증)
    └── TestMiniGitSRS 클래스 ────────────────────── [이론 19] 단위 테스트 (Unit Testing) 자동화
```

---

## 3. 각 파일 및 메서드별 실제 소스 코드 매핑 (접기/펼치기)
...
def three_way_merge_file_meta(base, mine, theirs):
    ...
    for key in all_keys:
        ...
        # Case 1: Mine과 Theirs의 결과가 동일함 (둘 다 변경 없음, 동일한 내용 수정/추가, 동일한 삭제 포함)
        if val_mine == val_theirs:
...
def quick_sort_inplace(arr, low, high):
    if low < high:
        # 분할(Partition) 과정을 통해 피벗의 올바른 위치를 찾고 인덱스를 반환받습니다.
        pivot_idx = partition(arr, low, high)
        # 피벗을 기준으로 왼쪽 영역과 오른쪽 영역을 재귀적으로 정렬합니다.
...
def partition(arr, low, high):
    # 피벗을 가장 오른쪽 원소로 선택
    pivot = arr[high]
    i = low - 1  # 피벗보다 작은 원소 영역의 경계 인덱스
    
    for j in range(low, high):
        if arr[j] < pivot:
            i += 1
            # 피벗보다 작은 원소를 왼쪽 영역으로 스왑(Swap)
...
quick_sort_inplace(test_list, 0, len(test_list) - 1)
print("퀵 정렬 결과:", test_list)
```

</details>

#### 4) 코드 동작 설명
* 추가적인 임시 배열 공간을 새로 생성하지 않고, 피벗보다 작은 요소들을 앞쪽으로 모으고 큰 요소들을 뒤쪽으로 보내는 스왑(Swap) 연산의 `partition` 과정을 통해 제자리(In-place) 정렬을 완성합니다.
* 분할된 피벗의 인덱스(`pivot_idx`)를 기준으로 좌우 하위 배열 범위(`low`에서 `pivot_idx - 1` 및 `pivot_idx + 1`에서 `high`)에 대해 함수를 재귀 호출하여 분할 정복을 진행합니다.
...
### [이론 15] 객체 불변성 (Object Immutability) 및 데이터 무결성 (Data Integrity)

#### 1) 배경지식이 없는 분들을 위한 개념 설명
* **불변성 (Immutability)**: 객체가 생성된 후 그 내부 상태(데이터)를 절대 변경할 수 없게 만드는 프로그래밍 설계 기법입니다.
* **데이터 무결성**: 데이터가 보관되는 과정에서 인위적이거나 뜻하지 않은 오류 등으로 인해 변경되거나 훼손되지 않고 정확성과 일관성을 그대로 유지하는 성질입니다.
* **실생활 비유**: **도장 찍은 공증 계약서**와 같습니다. 계약을 체결하고 도장을 찍는 순간 그 계약서의 내용은 그 누구도 임의로 수정할 수 없으며(불변성), 만약 누군가 내용을 고치려 한다면 무효가 됩니다.

#### 2) 사용하는 이유와 Git에서의 역할
* **부작용(Side Effects) 차단**: 여러 스레드나 모듈에서 하나의 객체를 동시에 참조할 때, 한쪽에서 값을 바꿔 다른 쪽이 오작동하는 버그를 원천 봉쇄합니다.
* **커밋의 역사 보존**: Git에서 커밋은 한 번 생성되면 절대로 수정되어서는 안 되는 역사의 기록입니다. 이를 위해 `Commit` 객체는 파이썬에서 `@dataclass(frozen=True)`를 적용하여 강제로 필드 수정을 차단합니다.
...
# frozen=True 설정을 통해 불변 객체로 정의합니다.
@dataclass(frozen=True)
class ImmutableCommit:
    hash: str
    message: str

commit = ImmutableCommit("1", "Initial Commit")
print("생성된 커밋 메시지:", commit.message)

try:
    # 값 수정을 시도합니다.
    commit.message = "Modified Message"
except Exception as e:
    print("수정 실패 (오류 발생):", type(e).__name__, "-", e)
```

</details>

#### 4) 코드 동작 설명
* `@dataclass(frozen=True)` 데코레이터가 적용된 클래스는 인스턴스화된 후 내부적으로 속성을 수정하거나 삭제하려 할 때 `FrozenInstanceError` 예외를 발생시켜 객체의 불변성을 지킵니다.

#### 5) 🎓 [교수-제자 모의 토론 보충설명]
* **학생**: 교수님, 불변 객체의 내용을 수정하고 싶을 때는 어떻게 해야 하나요?
* **교수**: 좋은 질문이네. 불변 객체는 직접 고칠 수 없으므로, 기존 객체의 데이터를 복사하면서 원하는 값만 변경한 '완전히 새로운 객체'를 생성해서 리턴해야 하네. 이를 파이썬에서는 `dataclasses.replace(obj, ...)` 함수를 통해 우아하게 처리한다네.

---

### [이론 16] 소프트웨어 디자인 패턴 - 데코레이터 패턴 (Decorator Pattern)

#### 1) 배경지식이 없는 분들을 위한 개념 설명
* **데코레이터 패턴**: 기존에 작성된 함수나 클래스의 소스 코드를 단 한 줄도 건드리지 않고, 새로운 기능(로그 출력, 권한 검사, 캐싱 등)을 겉에 씌워 추가하는 설계 패턴입니다.
* **실생활 비유**: **스마트폰의 투명 범퍼 케이스**와 같습니다. 스마트폰 자체의 하드웨어를 뜯어내지 않고도 케이스를 끼우는 행위만으로 '충격 방지'나 '그립감 개선'이라는 부가 기능을 덧붙일 수 있습니다.

#### 2) 사용하는 이유와 Git에서의 역할
* **단일 책임 원칙(SRP) 및 코드 재사용**: 특정 핵심 비즈니스 로직(예: 명령어 실행)과 이를 실행하기 전후에 거쳐야 하는 공통 검증 코드(예: 저장소 초기화 여부 확인)를 분리하여 코드 스파게티화를 방지합니다.
* **저장소 초기화 검증**: `main.py`에서 `@require_init` 및 `@args_to_list` 같은 데코레이터를 사용하여, 사용자가 명령어를 입력할 때마다 `init` 상태인지 검증하고 매개변수를 전처리하는 횡단 관심사(Cross-cutting Concerns)를 명확하게 분리 처리합니다.
...
# 저장소 초기화 여부를 확인하는 데코레이터 정의
def require_init(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        is_initialized = True  # 가상의 초기화 여부 확인
        if not is_initialized:
            print("에러: 저장소가 초기화되지 않았습니다.")
            return
...
#### 4) 코드 동작 설명
* 데코레이터는 대상 함수를 인자로 받아 내부 래퍼 함수(`wrapper`)로 감싸서 반환합니다. 파이썬의 `@` 문법 설탕을 통해 해당 함수 호출 시 실제로는 래퍼 함수가 실행되며 사전/사후 로직을 횡단 처리하게 됩니다.

---

### [이론 17] 프레젠테이션 레이어 분리 및 MVC (Presentation-Domain Separation)

#### 1) 배경지식이 없는 분들을 위한 개념 설명
* **레이어 아키텍처 (Layered Architecture)**: 프로그램의 역할을 성격에 따라 구획별로 나누어 층(Layer)을 쌓는 구조적 설계 방식입니다.
* **프레젠테이션 레이어(화면/입출력)**: 사용자에게 정보를 보여주고 키보드 입력을 받는 영역입니다.
* **도메인/비즈니스 레이어(로직/계산)**: 화면 출력엔 신경 쓰지 않고 데이터의 논리적인 연산과 비즈니스 룰을 처리하는 알맹이 영역입니다.
* **실생활 비유**: **식당의 서빙 직원(홀)과 요리사(주방)의 분리**와 같습니다. 서빙 직원은 손님의 언어를 알아듣고 주문을 접수하며 요리를 예쁘게 내갑니다. 요리사는 손님이 어떻게 생겼는지 관심 없고 오직 주문서에 맞춰 최고의 맛을 내는 요리를 만듭니다.

#### 2) 사용하는 이유와 Git에서의 역할
* **유지보수와 확장성**: 만약 화면 출력 코드와 핵심 알고리즘이 섞여 있으면, CLI 셸이 아니라 GUI(마우스 클릭 화면)로 바꿀 때 핵심 코드도 다 깨지게 됩니다. 
* **구조적 분리**: Mini Git 프로젝트에서는 `main.py`(`GitShell` 클래스 - REPL 파싱, 화면 출력, CLI 프롬프트 관리)가 프레젠테이션 역할을 하고, `mini_git.py`(`Git` 클래스 - 순수 버전 관리 알고리즘)가 도메인 엔진 역할을 맡도록 철저히 분리 설계되어 있습니다.
...
```python
# 1. 도메인 레이어 (출력/입력 코드가 없고 순수 논리 계산만 처리)
class CoreCalculator:
    def add(self, a, b):
        return a + b

# 2. 프레젠테이션 레이어 (사용자 입출력과 흐름만 관리)
class CalculatorCLI:
    def __init__(self):
    ...
    def run(self):
        print("=== 계산기 CLI ===")
        val1 = int(input("첫 번째 숫자: "))
        val2 = int(input("두 번째 숫자: "))
        result = self.calc.add(val1, val2)
...
#### 4) 코드 동작 설명
* 핵심 로직인 `CoreCalculator`는 사용자 화면의 존재를 전혀 모른 채 독립적으로 작동합니다. `CalculatorCLI`가 사용자로부터 데이터를 가공해 와서 핵심 로직을 대리 구동하고 결과를 다시 가공해 보여줍니다.

---

### [이론 18] 메모이제이션 및 캐싱 (Memoization & Caching)

#### 1) 배경지식이 없는 분들을 위한 개념 설명
* **캐싱 (Caching)**: 시간이 오래 걸리는 작업의 결과를 임시 보관함에 저장해 두었다가, 똑같은 계산 요청이 올 때 다시 계산하지 않고 즉시 꺼내주는 고속화 기법입니다.
* **메모이제이션**: 캐싱의 수학적 구현 형태로, 함수의 입력값에 따른 출력값을 맵(Dictionary) 등에 기억해 두는 행위입니다.
* **실생활 비유**: **자주 쓰는 번호 단축키 설정** 또는 **구구단 외우기**입니다. 7 * 8을 계산할 때 매번 7을 여덟 번 더하는 바보 같은 계산을 반복하지 않고, 뇌 속의 메모리(구구단 표)에서 56을 즉시 꺼내 대답하는 것입니다.

#### 2) 사용하는 이유와 Git에서의 역할
* **시간 복잡도 비약적 단축**: 커밋 그래프는 기본적으로 부모 방향으로만 연결(`Commit.parents`)되어 있어서 역방향(자식 방향)을 찾으려면 전체 그래프를 매번 훑어야 합니다.
* **자식 노드 캐시**: `CommitGraph`는 커밋이 추가될 때마다 `self.children_dict`라는 캐시 맵에 자식 관계를 실시간으로 기록(Memoization)해 둡니다. 덕분에 `find_shortest_path`에서 BFS로 무방향 탐색을 할 때 자식 노드를 O(1)의 상수 시간으로 탐색하여 전체 그래프 연산의 시간 성능을 극대화합니다.
...
```python
# 피보나치 수열 연산에서의 메모이제이션 예시
memo = {}

def fibonacci_memo(n):
    if n in memo:
        return memo[n]  # 이미 계산한 적이 있다면 캐시에서 즉시 반환 (O(1))
    if n <= 1:
        return n
...
print("피보나치 30번째 항:", fibonacci_memo(30))
```

</details>

#### 4) 코드 동작 설명
* `memo` 딕셔너리를 활용하여 중복된 계산 호출의 결괏값을 캐시합니다. 이 단순한 메모이제이션 기법 하나로 피보나치의 시간 복잡도는 지수 시간 O(2^N)에서 선형 시간 O(N)으로 비약적인 최적화를 이뤄냅니다.

---

### [이론 19] 단위 테스트 (Unit Testing)와 자동 검증

#### 1) 배경지식이 없는 분들을 위한 개념 설명
* **단위 테스트 (Unit Test)**: 프로그램의 전체 시스템을 통째로 돌려보는 대신, 아주 작게 쪼개진 단위(함수 하나, 클래스 메서드 하나)가 정해진 규격대로 완벽하게 작동하는지 개별 검증하는 소형 모의 테스트입니다.
* **테스트 자동화**: 사람이 직접 실행해 볼 필요 없이 컴퓨터가 수십, 수백 개의 테스트 시나리오를 알아서 정해진 규칙대로 실행하고 합격/불합격 판정을 내려주는 시스템입니다.
* **실생활 비유**: **스마트폰 출고 전 부품 검사**와 같습니다. 완성품을 들고 전화를 해보는 것도 중요하지만, 부품 조립 라인에서 액정의 터치 반응 속도가 10ms 이하인지, 스피커 진동수가 기준치에 맞는지 정밀 장비로 자동 측정하여 결함을 미리 걸러내는 일과 유사합니다.

#### 2) 사용하는 이유와 Git에서의 역할
* **회귀 버그(Regression Bug) 방지**: 코드를 일부분 리팩토링하거나 최적화했을 때 기존에 잘 작동하던 다른 기능이 연쇄적으로 고장 나는 현상을 예방합니다.
* **자동화된 코드 품질 보증**: `test_minigit.py`에 작성된 `TestMiniGitSRS` 테스트 스위트가 `unittest` 프레임워크를 기반으로 돌아가며 위상 정렬, 최단 경로, 3방향 병합 등 Mini Git의 모든 핵심 알고리즘이 수정 후에도 안전하게 작동하는지 검증합니다.
...
# unittest.TestCase를 상속하여 단위 테스트 클래스 정의
...
#### 4) 코드 동작 설명
* `unittest` 라이브러리는 `test_`로 시작하는 이름의 메서드들을 자동으로 수집하여 순차 실행합니다. 내부적으로 `assertEqual` 등의 단언문(Assertion)을 사용하여 함수의 실제 출력값과 기대값이 일치하지 않을 때 자동으로 실패를 감지해 빨간불을 켜줍니다.

---

### [이론 20] LCS (Longest Common Subsequence - 최장 공통 부분 수열)

#### 1) 배경지식이 없는 분들을 위한 개념 설명
* **최장 공통 부분 수열 (LCS)**: 두 개의 문자열이나 배열이 주어졌을 때, 양쪽 모두에 **공통적으로 등장하는 부분 수열(Subsequence) 중 가장 긴 것**을 찾는 알고리즘입니다.
* **부분 수열(Subsequence)의 규칙**: 문자열에서 중간 문자들을 건너뛰어도 되지만, **원래 문자열 내에서의 선후 순서(방향)는 반드시 유지**되어야 합니다.
  * 예: `ABCD`에서 `AC`는 중간에 `B`를 건너뛰었지만 순서가 맞으므로 부분 수열입니다. 반면 `CA`는 순서가 뒤집혔으므로 부분 수열이 아닙니다.
* **실생활 비유**: **두 아이의 그림 맞추기 또는 DNA 서열 대조**와 같습니다.
  * A 아이의 장난감 나열: `[자동차, 로봇, 인형, 기차]`
  * B 아이의 장난감 나열: `[로봇, 자동차, 인형, 비행기]`
  * 순서를 지키면서 공통으로 일치하는 가장 긴 장난감 세트는 무엇일까요? `[로봇, 인형]` (길이 2) 또는 `[자동차, 인형]` (길이 2) 입니다.

#### 2) 사용하는 이유와 Git에서의 역할
* **차이점(Diff) 연산의 뼈대**: Git에서 두 파일의 차이점(`git diff`)을 보여줄 때, 무엇이 추가되었고 무엇이 삭제되었인지 기준을 잡아야 합니다.
* **공통 기준선 설정**: 파일 A와 파일 B의 공통 분모인 LCS를 찾아내면, LCS에 속하지 않은 부분들을 각각 **"A에서 삭제된 것(-)"**과 **"B에 추가된 것(+)"**으로 명확히 규정할 수 있습니다. 이를 통해 두 파일의 버전 간 변화를 줄 단위로 시각화합니다.
...
```python
def find_lcs_and_diff(list_a, list_b):
    # A와 B의 크기
    m, n = len(list_a), len(list_b)
    
    # 1. DP(동적 계획법) 테이블 생성 및 0으로 초기화 (m+1 x n+1 크기)
    # dp[i][j]는 list_a[:i]와 list_b[:j]의 LCS 길이를 저장합니다.
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            if list_a[i - 1] == list_b[j - 1]:
                # 두 원소가 같으면, 대각선 왼쪽 위 값에 1을 더합니다.
                dp[i][j] = dp[i - 1][j - 1] + 1
            else:
                # 다르면, 위쪽 칸과 왼쪽 칸 중 큰 값을 가져옵니다.
                dp[i][j] = max(dp[i - 1][j], dp[i][j - 1])
                
    # 3. DP 테이블을 역추적(Backtracking)하여 Diff 결과 생성
    diff_result = []
    i, j = m, n
    ...
    while i > 0 or j > 0:
        if i > 0 and j > 0 and list_a[i - 1] == list_b[j - 1]:
            # 공통 줄인 경우 (변화 없음)
            diff_result.append(f"  {list_a[i - 1]}")
            i -= 1
            j -= 1
        elif j > 0 and (i == 0 or dp[i][j - 1] >= dp[i - 1][j]):
            # B에만 있는 원소 -> 추가(+)로 판단
            diff_result.append(f"+ {list_b[j - 1]}")
            j -= 1
    ...
    # 역추적을 했으므로 결과를 뒤집어서 반환합니다.
    diff_result.reverse()
    return dp[m][n], diff_result

# 테스트 데이터 (두 버전의 텍스트 파일)
file_version_1 = ["hello", "world", "learn git", "exit"]
file_version_2 = ["hello", "beautiful", "world", "learn git", "quit"]

lcs_len, diff = find_lcs_and_diff(file_version_1, file_version_2)

print(f"최장 공통 부분 수열(LCS) 길이: {lcs_len}")
print("=== Diff 결과 ===")
for line in diff:
    print(line)
```
...
#### 4) 코드 동작 설명
* **동적 계획법 (Dynamic Programming)**: 큰 문제를 작은 하위 문제로 분할하고, 이미 계산한 하위 문제의 답을 표(`dp` 리스트)에 기록(Memoization)해 재활용하는 최적화 기법입니다.
* **테이블 갱신 규칙**:
  * 비교하는 원소가 일치하면: 이전 원소까지의 LCS 길이에 1을 더합니다. ($dp[i][j] = dp[i-1][j-1] + 1$)
  * 일치하지 않으면: 한쪽 문자열만 직전 상태였을 때의 누적 LCS 중 큰 쪽을 계승합니다. ($dp[i][j] = \max(dp[i-1][j], dp[i][j-1])$)
* **역추적 (Backtracking)**: 테이블의 가장 우측 하단($dp[m][n]$)에서 출발하여, 누적 값이 깎여 나간 원인을 추적합니다. 대각선 이동은 공통(유지), 수평/수직 이동은 추가/삭제로 판별하여 최종 줄 비교 결과를 도출합니다.

#### 5) 🎓 [교수-제자 모의 토론 보충설명]
* **학생**: 교수님, LCS의 시간 복잡도는 $O(M \times N)$인데, 대용량 파일 두 개를 비교할 때는 너무 느려지지 않나요?
* **교수**: 아주 날카로운 지적일세. 소스 코드 파일이 수만 줄이 되면 $O(M \times N)$의 DP 테이블은 엄청난 시간과 메모리를 소모한다네. 그래서 실제 Git의 내부 `git diff` 명령어는 고전적인 LCS DP 알고리즘을 그대로 쓰지 않고, Myers의 탐색 알고리즘(Myers' Diff Algorithm)이라는 탐색 공간 최적화 기법을 사용하네. 이는 최단 경로 탐색(BFS)의 변형으로, 실질적으로 변경 사항이 많지 않은 대부분의 경우 거의 $O(N + D^2)$ (D는 차이점의 수)에 준하는 엄청나게 빠른 속도로 차이점을 추출해 준다네.
