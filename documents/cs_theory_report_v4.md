...
본 보고서는 `cs_theory_report_v3.md` 문서를 기반으로 작성된 **버전 4 최종 확장판**입니다. 컴퓨터 사이언스(CS) 배경지식이 없는 고등학생도 각 이론의 원리와 역할을 쉽게 파악할 수 있도록 친절한 비유를 대폭 보강하였으며, **`simple_1` 프로젝트에 사용되었으나 v3 문서에서 누락되었던 5가지 핵심 CS 이론**을 신규 통합하였습니다.

또한, 실제 소스 코드 매핑 및 세부 내용(접기/펼치기)은 가독성과 중복 최소화를 위해 별도 문서인 [simple_1_code_chunk_mapping.md](simple_1_code_chunk_mapping.md) 파일로 분리하여 구성하였습니다.

---

## 1. Mini Git CS 이론 위치 매핑 트리 구조

다음 트리는 `simple_1` 구현물 내 각 파일의 클래스 및 메서드에 어떤 CS 이론이 밀접하게 연계되어 작동하는지 매핑한 구조입니다.
...
👉 **[simple_1_code_chunk_mapping.md](simple_1_code_chunk_mapping.md)**
...
* **실제 구현과의 매핑**: `simple_1` 프로젝트의 `Commit_graph.get_all_ancestors` 메서드는 과거 재귀 구현체에서 발생 가능한 Stack Overflow 버그를 해결하기 위해, 위 소스 코드처럼 **스택을 사용하는 반복적 DFS 방식**으로 완벽하게 수정되어 있습니다.
...
* **실제 구현과의 매핑**: `simple_1` 프로젝트의 `Git.log_sorted` 메서드에서는 내장 정렬(`sorted()`)을 일절 배제하고 최악과 평균 시간복잡도 $O(N \log N)$을 보장하는 이 **병합 정렬(Merge Sort)** 방식을 안정적으로 커스텀 구현하여 사용하고 있습니다.
...
* **실제 구현과의 매핑**: `simple_1` 프로젝트의 `Commit_graph` 생성 시 `commit_word_dict`(단어 색인)와 `commit_author_dict`(작성자 색인)를 구비하여 커밋 시점에 메시지를 토큰화해 매핑하고 있습니다.
...
* **실제 구현과의 매핑 & 대조**: 
  * 본 `simple_1` 프로젝트에서는 모든 커밋 노드를 파이썬 내장 해시 테이블 사전형태(`dict`)인 `commit_dict`에 적재하여 조회합니다.
...
* **실제 구현과의 매핑**: `simple_1` 프로젝트의 `main.py` 파일 내에서는 이 상태 기계를 매번 직접 돌리는 대신, 파이썬 표준 라이브러리인 **`shlex.split(line)`** 모듈을 호출하여 동일한 어휘 분석 토큰 파싱을 수행하고 있습니다.
...
* **실제 구현과의 매핑**: `simple_1` 프로젝트의 `three_way_merge`와 `finalize_merge` 메서드가 이 삼대면 로직을 정확히 내포하고 있으며, `find_common_ancestor`가 역방향 BFS 탐색을 처리하고 있습니다.
