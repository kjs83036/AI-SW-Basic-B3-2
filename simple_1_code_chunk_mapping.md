# Mini Git (`simple_1`) 소스 코드 & CS 이론 매핑 가이드 (주석 임베디드 버젼)

이 가이드는 `simple_1` 프로젝트의 실제 파이썬 소스 코드 내부에 **컴퓨터 사이언스(CS) 이론이 적용된 지점을 주석(`# [이론 X] ...`) 형태로 표시**하고, 코드를 펼치기 전인 **접기 상태의 요약 라인(`<summary>`)에서도 어떤 이론이 사용되었는지 한눈에 파악**할 수 있도록 구성했습니다.
...
### 📂 [commit_graph.py](simple_1/commit_graph.py)
...
### 📂 [mini_git.py](simple_1/mini_git.py)
